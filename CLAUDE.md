...
reference_importer.import_low_buffer `:45`; чистка `awaiting_media` `:15`; **process_due_ads
...
- **Добор буфера и приоритет источника (важно):** общий целевой уровень — `BUFFER_TARGET`=5
  (config; и генерация, и референсы добивают очередь ДО него, не переполняя). Источник — по
  приоритету на канал: **есть `reference_channels` (донор) → добор ТОЛЬКО с донора**
  (`import_low_buffer` `:45`); если донор пуст и буфер всё ещё <`LOW_BUFFER_MIN`=5 — **фолбэк**:
  добиваем генерацией (marketplace → WB) через `generator.run_for_channel`. Каналы БЕЗ донора
  обслуживает `run_top_up_cycle` `:30` (генерация; marketplace → WB), и она **пропускает
  донор-каналы** (иначе двойной залив). Слепого ежедневного импорта нет — ручной долив кнопкой
  «📥 Взять»; `import_all` оставлен для ручного «импортнуть всё». **Черновики авто-добор НЕ
  трогает** (ручные, могут быть не готовы).
...
Живой план работ и приоритеты — в `BACKLOG.md`. Постановки — `SAAS_SPEC.md`/`COST_SPEC.md`.
