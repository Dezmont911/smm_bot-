...
CELEB_DRAMA_PROFILE_MARKERS = (
    ...
    "инстаграм", "instagram", "facebook", "фейсбук", "вконтакте", "vk", "max", "макс",
    "фанат", "подписчик", "роман", "расставан", "скандал",
...
IMPORT_AD_MARKERS = (
    ...
    "мы в max", "мы в макс", "подпишись в max", "подпишись в макс",
    "подписывайся в max", "подписывайся в макс", "читайте нас в max",
    "читайте нас в макс", "наш max", "наш макс",
)

IMPORT_AD_WORD_RE = re.compile(
    r"(?iu)\b(розыгрыш\w*|разыгр\w*|конкурс\w*)\b"
)
...
def _has_broad_fact_advice_drift(text: str) -> bool:
    ...
    if re.search(
        ...
        r"обсуди|тестируй|сравни|записывай|фотографируй|собери|посади|наблюдай|укрепи|настрой|проверь)\b",
        low,
    ):
        return True
    ...
    if re.search(
        ...
        r"записывай|фотографируй|настрой|проверь|используй)\b",
        low,
    ):
        return True

    practical_patterns = (
        r"(?iu)\b(лайфхак|практическ\w*\s+польз\w*|это\s+полезно\s*:)",
        r"(?iu)\b(как\s+использовать|использовать\s+открытие|используй\s+открытие)\b",
        r"(?iu)\b(домашн\w*\s+исследован\w*|в\s+быту|для\s+быта|для\s+дома)\b",
        r"(?iu)\b(сад\s+у\s+дома|огород|рассад\w*|посади|террас\w*|подпорн\w*\s+стен\w*|дождев\w*\s+вод\w*)\b",
        r"(?iu)\b(лаборатор\w*.*установк\w*|установк\w*.*лазер\w*|контрольн\w*\s+образц\w*|настрой\w*.*контраст)\b",
    )
    if any(re.search(pattern, low) for pattern in practical_patterns):
        return True
...
def build_content_brief(channel: dict, safety: dict, format_name: str | None = None) -> dict:
    ...
    if _is_broad_fact_channel(channel) and channel.get("allow_advice_format") is not True:
        must_avoid.extend([
            "не писать советы, лайфхаки и пошаговые инструкции",
            "не превращать факт в бытовую пользу, сад/огород, домашний эксперимент или лабораторную инструкцию",
            "не начинать пост с 'Узнай как', 'Используй', 'Попробуй', 'Сделай', 'Найди'",
            "писать как короткую познавательную заметку: факт + почему это интересно, без призыва что-то делать",
        ])
