...
_FORMAT_LABEL_PREFIX_RE = re.compile(
    r"^\s*(?:\*\*)?\s*(?:факт|совет|вопрос|разбор|мини-разбор|инфоповод|история|лайфхак)\s*(?:\*\*)?\s*[:—–-]\s*(?:\*\*)?\s*",
    re.IGNORECASE,
)
...
def _clean_post_output(content: str) -> str:
    ...
    for _ in range(2):  # до двух вступлений подряд
        ...
        t = new
    for _ in range(2):
        new = _FORMAT_LABEL_PREFIX_RE.sub("", t, count=1).strip()
        if new == t:
            break
        t = new
...
_CONTENT_FORBIDDEN_STEMS = (
    ...
    "генитал", "куннилингус", "совокупл", "хентай", "hentai", "нюдес", "нюдс",
    "nude", "nudes", "фетиш", "fetish",
)
...
DEFAULT_FORBIDDEN_TOPICS = [
    "политика", "18+", "наркотики", "азартные игры", "порно", "хентай", "нюдесы",
    "фетиши", "война",
...
- Не начинай пост с названия формата: "Факт:", "Совет:", "Разбор:", "Вопрос:", "Инфоповод:".
...
без названия формата в начале («Факт:», «Совет:», «Разбор:», «Вопрос:», «Инфоповод:»),
