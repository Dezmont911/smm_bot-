...
CELEB_DRAMA_RETRY_FOCUSES = (
    (
        "Ищи не в общей политической ленте, а в lifestyle/entertainment/media/marketing "
        "источниках. Подходят запросы: блогер запустил проект, блогер коллаборация, "
        "инфлюенсер новый бренд, блогер Forbes рейтинг, YouTube блогер интервью, "
        "стример новый проект, блогер сменил образ."
    ),
    (
        "Сфокусируйся на мягких инфоповодах без судов и уголовки: личные события, "
        "отношения, карьера, шоу, интервью, контент-проекты, рост аудитории, "
        "запуски курсов/брендов/каналов. Не бери темы из криминальной, политической "
        "и юридической повестки."
    ),
)
...
def _build_prompt(channel: dict, count: int, used_topics: list[str]) -> str:
    ...
    archetype = str(channel.get("archetype") or "").strip().lower()
    ...
    niche_rules = ""
    if archetype == "celeb_drama":
        niche_rules = """
Для этого канала подходят ТОЛЬКО конкретные новости о конкретных блогерах, стримерах,
инфлюенсерах, артистах или интернет-персоналиях.
Каждая тема должна содержать имя/ник/проект + конкретное событие:
Forbes/рейтинг, рост аудитории, новый проект, шоу, клип, коллаборация, интервью,
роман, расставание, смена образа, фешн/путешествие, публичный конфликт без политики,
реакция фанатов, запуск канала/бренда/курса/медиа-проекта.

Ищи в первую очередь мягкие creator-news: YouTube, Twitch, TikTok, Telegram, Instagram,
инфобизнес, шоу-бизнес, блогерские рейтинги, личные события, коллаборации, контент-проекты.
Не ищи по общей новостной повестке. Нужны темы, которые можно написать как пост
про блогеров/инфлюенсеров, а не как сводку происшествий или политики.

НЕ предлагай:
- абстрактные темы вроде "личная жизнь блогеров", "мифы вокруг инфлюенсеров",
  "стоит ли блогерам быть честными", "канал публикует новости..."
- иноагентов, Минюст, суды, штрафы, колонии, УДО, уголовные дела, аресты, следствие, приговоры
- общие политические/военные/медицинские/криминальные новости, даже если там упомянут блогер
- обычные новости России/мира, даже если их можно притянуть к реакции блогеров
"""
    example = (
        '["Максим Лутчак попал в Forbes 30 до 30 после роста аудитории", '
        '"Фешн-блогер Игор Синяк рассказал об ограблении в Париже"]'
        if archetype == "celeb_drama"
        else '["Valve выпустила обновление X для CS2", "Команда Y выиграла мажор Z"]'
    )
...
async def get_topics(channel: dict, count: int, used_topics: list[str] | None = None) -> list[str]:
    ...
    archetype = str(channel.get("archetype") or "").strip().lower()
    if archetype == "celeb_drama" and len(fresh) < need:
        seen = {t.strip().lower() for t in picked + fresh}
        retry_count = max(batch, need * 8, 24)
        for focus in CELEB_DRAMA_RETRY_FOCUSES:
            retry_channel = dict(channel)
            base_topic = str(channel.get("topic") or "").strip()
            retry_channel["topic"] = f"{base_topic}\n\nДополнительный поисковый фокус: {focus}"
            retry_topics = await discover_topics(
                retry_channel,
                count=retry_count,
                used_topics=used_topics + picked + fresh,
            )
            retry_topics = _filter_topics_for_channel(retry_channel, retry_topics, "search_retry")
            added = 0
            for topic in retry_topics:
                ...
                if len(fresh) >= need:
                    break
            logger.info(
                f"Темы [{channel_id}]: celeb retry добавил {added}, всего после фильтра={len(fresh)}"
            )
            if len(fresh) >= need:
                break
