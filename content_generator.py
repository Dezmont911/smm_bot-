...
def _fallback_angles_for_channel(channel: dict) -> list[str]:
    """Safe synthetic topic angles when external sources do not provide enough topics."""
    if _is_broad_fact_channel(channel):
        return [
            "короткий неочевидный факт и почему он интересен",
            "научное открытие простыми словами без инструкций",
            "редкий факт о природе или животных",
            "неожиданная деталь о человеческом организме",
            "исторический факт без политики",
            "факт о привычной вещи, который мало кто знает",
            "объяснение природного явления без инструкций",
            "миф и реальность без практических инструкций",
            "цифра или статистика, которая удивляет",
            "факт о природе, который звучит как выдумка",
            "наблюдение о поведении людей или животных без инструкций",
            "короткое объяснение сложного механизма",
            "сравнение двух похожих явлений как познавательный факт",
            "что происходит за кадром обычного процесса без бытовых рекомендаций",
            "мини-заметка для расширения кругозора",
            "один факт и простое объяснение его смысла",
        ]

    return [
        "интересный неочевидный факт",
        "разбор для новичка",
        "частая ошибка и как её избежать",
        "практический совет из опыта",
        "мифы и правда",
        "свежий взгляд на привычное",
        "топ-подборка по теме",
        "короткая история из практики",
        "удивительное наблюдение из мира животных",
        "редкий факт о растениях и экосистемах",
        "неожиданная деталь о человеческом организме",
        "научное открытие простыми словами",
        "исторический факт без политики",
        "факт о привычной вещи, который мало кто знает",
        "объяснение природного явления",
        "ошибка восприятия, из-за которой люди делают неверный вывод",
        "мини-проверка популярного мифа",
        "познавательный факт для короткой заметки",
        "сравнение двух похожих явлений",
        "что происходит за кадром обычного процесса",
        "цифра или статистика, которая удивляет",
        "безопасный факт без войны и политики",
        "вопрос, который заставляет посмотреть на тему иначе",
        "практическая польза знания из науки",
        "простое объяснение сложного механизма",
        "подборка из трех коротких фактов",
        "факт о природе, который звучит как выдумка",
        "наблюдение о поведении людей или животных",
...
class ContentGenerator:
    ...
    async def run_for_channel(
        ...
        for topic_data in topics:
            ...
            try:
                ...
                if self._topic_already_used(
                    safe_topic,
                    used_topics,
                    fuzzy=topic_data.get("source") != "fallback",
                ):
                    logger.info(
    ...
    async def _collect_topics(
        ...
        if len(topics) < count:
            ...
            if base:
                ANGLES = _fallback_angles_for_channel(channel)
    ...
    def _topic_already_used(self, topic: str, used_topics: list[str], *, fuzzy: bool = True) -> bool:
        """
